 ...
 * It opens a server socket on a given port and waits for a client to connect using an infinite loop. Once a client is
 * connected, this app reads a message from the connected socket input stream and prints it to the console.
 * <p>
 * It uses only one thread. So if multiple clients try to connect at the same time, they will be waiting and will
...
public class MessageBoardServer {
    ...
    public static void main(String[] args) throws IOException {
        try (ServerSocket serverSocket = createServerSocket(PORT)) {
            System.out.println("Message Board Server started on " + HOST + ":" + PORT);
