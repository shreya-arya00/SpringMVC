...
public class ClientUtil {
    ...
    public static Socket openSocket(String host, int port) {
        try {
            ...
            throw new IOException("Failed to open socket to " + host + ":" + port, e);
        }
    }
     ...
     * it flushes the buffer of the writer using a corresponding method.
     *
    ...
    public static void writeToSocket(String message, Socket socket) {
        try (BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(socket.getOutputStream()))) {
            ...
            writer.newLine();
            writer.flush(); // Ensure the message is sent by flushing the buffer
        } catch (IOException e) {
            throw new IOException("Failed to write message to socket", e);
        }
    }
}
